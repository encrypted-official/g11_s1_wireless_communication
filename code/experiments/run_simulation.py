...
Group 11, Milestone 3

PIPELINE (executed explicitly in run_single_trial()):
    Step 1 → Generate OFDM signal  s[n]
    Step 2 → Generate Gaussian noise  w[n]
    Step 3 → Form received signal  x[n] = s[n] + w[n]
    Step 4 → Compute cyclic autocorrelation  R̂_x^α(τ)
    Step 5 → Compute multi-cycle test statistic  Tmc
    Step 6 → Compare with threshold  λ
...
# ════════════════════════════════════════════════════════════════════════════
# TRIAL RESULT  (stores every intermediate value from one pipeline run)
# ════════════════════════════════════════════════════════════════════════════
@dataclass
class TrialResult:
    s:           np.ndarray   # Step 1 — OFDM signal
    w:           np.ndarray   # Step 2 — Gaussian noise
    x:           np.ndarray   # Step 3 — received signal
    numerator:   float        # Step 4 — CAC numerator
    denominator: float        # Step 4 — CAC denominator
    tmc:         float        # Step 5 — test statistic
    threshold:   float        # Step 6 — threshold λ
    decision:    int          # Step 7 — 0=H0, 1=H1
    hypothesis:  str          # ground truth


# ----------------------------------------------------------------------------
# CORE PIPELINE FUNCTION  ← every step lives here, in order
# ----------------------------------------------------------------------------
def run_single_trial(
    ...
    w = generate_noise_with_uncertainty(
        num_samples=N,
        nominal_variance=noise_variance,
        uncertainty_db=noise_uncertainty_db,
        rng=rng,
    )

    # ── STEP 3: Form received signal x[n] ──────────────────────────────────
    if hypothesis == "H1":
        x = s + w          # signal present
    else:
        x = w.copy()       # noise only

    # ── STEP 4: Compute cyclic autocorrelation ──────────────────────────────
    ...
    # ── STEP 6: Compare Tmc with threshold λ ───────────────────────────────
    # (comparison is done inside step 7 below)

    # ── STEP 7: Deterministic decision rule ────────────────────────────────
    #   H1  if  Tmc >= λ
    #   H0  if  Tmc <  λ
...
# ════════════════════════════════════════════════════════════════════════════
# STEP 8: Monte Carlo — estimate Pf and Pd over many trials
# ════════════════════════════════════════════════════════════════════════════
...
def plot_pipeline_diagram(demo: TrialResult, save_path: str):
    """Render a visual pipeline diagram annotated with actual values."""
    fig, ax = plt.subplots(figsize=(14, 6))
    ax.set_xlim(0, 14)
    ax.set_ylim(0, 6)
    ax.axis("off")
    fig.suptitle(
        "Detection Pipeline — Multi-Cycle Cyclostationary Detector  (CSE400, Milestone 3)\n"
    ...
    palette = ["#1e3a5f", "#1e3a5f", "#1e3a5f",   # steps 1-3 (signal chain)
               "#065f46", "#065f46",                # steps 4-5 (feature extraction)
               "#7c1d1d", "#7c1d1d"]               # steps 6-7 (decision)
    ...
    for i, ((label, title, note), color) in enumerate(zip(steps, palette)):
        ...
        ax.text(xc, yb + bh + 0.13, label,
                ha="center", va="bottom", fontsize=7.5,
                color=color, fontweight="bold")
        ax.text(xc, yb + bh / 2 + 0.1, title,
                ha="center", va="center", fontsize=8.5,
                color=color, fontweight="bold", multialignment="center")
        ax.text(xc, yb - 0.22, note,
        ...
        if i < len(steps) - 1:
            ax.annotate(
                "", xy=(xc + bw/2 + gap, yb + bh/2),
                xytext=(xc + bw/2, yb + bh/2),
    ...
    legend_handles = [
        mpatches.FancyBboxPatch((0, 0), 1, 1, boxstyle="round,pad=0.1",
                                fc="#1e3a5f18", ec="#1e3a5f", lw=1.2, label="Signal Chain  (Steps 1–3)"),
        mpatches.FancyBboxPatch((0, 0), 1, 1, boxstyle="round,pad=0.1",
                                fc="#065f4618", ec="#065f46", lw=1.2, label="Feature Extraction  (Steps 4–5)"),
        mpatches.FancyBboxPatch((0, 0), 1, 1, boxstyle="round,pad=0.1",
                                fc="#7c1d1d18", ec="#7c1d1d", lw=1.2, label="Decision  (Steps 6–7)"),
    ]
...
def plot_tmc_distribution(tmc_h0, tmc_h1, threshold, save_path):
    ...
    ax.hist(tmc_h0[tmc_h0 <= clip], bins=bins, density=True,
            alpha=0.6, color=COLORS["noise"], label="H₀: noise only")
    ax.hist(tmc_h1[tmc_h1 <= clip], bins=bins, density=True,
            alpha=0.6, color=COLORS["ofdm"],  label="H₁: signal + noise")
    ax.axvline(threshold, color=COLORS["threshold"], linestyle="--",
               linewidth=2, label=f"λ = {threshold:.2f}")
...
def plot_pd_vs_snr(snr_db, pd_multi, pd_single, target_pfa, save_path):
    ...
    ax.set_title(f"Step 8 — Pd vs SNR  (Monte Carlo, Pf = {target_pfa})")
...
def run_simulation():
    print("\n" + "=" * 65)
    print("  CSE400 — Milestone 3: Spectrum Sensing Simulation")
    ...
    # ── DEMO: one full pipeline pass, printed step by step ──────────────────
    print("\n" + "─" * 65)
    ...
    snr_mc_lin = 10**(-10/10)
    print(f"\n[Step 8a] Monte Carlo  ({num_trials} trials per hypothesis, SNR = −10 dB)...")
    pf_est, pd_est, tmc_h0_vals, tmc_h1_vals = monte_carlo_metrics(
    ...
    print(f"  Empirical Pf = {pf_est:.4f}   (theory = {1/(threshold+1):.4f})")
    print(f"  Empirical Pd = {pd_est:.4f}   at SNR = −10 dB")
    ...
    # ── Step 8b: SNR sweep ────────────────────────────────────────────────────
    print(f"\n[Step 8b] SNR sweep  ({len(snr_range_db)} points × {num_trials} trials)...")
